...
def test_parse_xml_to_dict(file_location):
    ...
    assert bible_dict["Psalms"][16][11]["text"] == VERSE_TEXTS["Psalm 16:11"]
    assert (
        bible_dict["Ephesians"][2][8]["text"] == VERSE_TEXTS["Ephesians 2:8"]
    )
    assert (
    ...
    assert (
        bible_dict["Proverbs"][18][17]["text"] == VERSE_TEXTS["Proverbs 18:17"]
    )
